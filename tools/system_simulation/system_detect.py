"""
Small utility which can be used to send out a simulated heart beat out once.
The user provides the device that is being simulated using a CANopen node,
and the result is the associated heart beat.
...
def main():
    argparser = ArgumentParser(description='''Utility to simulate a heart beat''')
    ...
    argparser.add_argument('device', action='store', type=int, help='''The
                           CANopen node ID to simulate the heart beat of''')
